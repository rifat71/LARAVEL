...
$("button").click(function(){
	...
	$.ajax({
		url:"/receptionist/searchByName/" + $('#name').val(),
		method:"get",
		success:function(result){
			
			//alert(result.length);
			data ="<table border='1'> <tr> <th>ID</th> <th>Username</th> <th>Phone</th> <th>Address</th> </tr>";
			for(var i=0;i<result.length;i++){
				data += "<tr> <td>"+result[i].id+"</td> <td>"+result[i].rusername+"</td> <td>"+result[i].rphone+"</td> <td>"+result[i].raddress+"</td> </tr>";
				//document.write(res);
			}
			data +="</table>";
			$("#search").html(data);
